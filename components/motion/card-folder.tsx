...
export function CardFolder({
  ...
  const defaultAriaLabel = front
    ? `${isOpen ? "Close" : "Open"} ${title}`
    : `${isOpen ? "Close" : "Open"} ${title}, card ending in ${visibleLastFour}, expires ${expiry}`;
  ...
  return (
    ...
    >
      ...
      <motion.span
        ...
        className={cn(
          "pointer-events-none absolute inset-x-0 bottom-0 top-1/2 z-20 [backface-visibility:hidden] [transform-origin:center_bottom]",
          frontClassName,
        )}
      >
          <svg
          ...
          >
            <path
              d={frontShape === "flat"
                ? "M16 4H368C377 4 384 11 384 20V94C384 103 377 110 368 110H16C7 110 0 103 0 94V20C0 11 7 4 16 4Z"
                : "M0 17C15 7 31 4 49 4H87C110 4 126 17 144 32L158 44C176 59 206 59 225 43L240 30C257 16 271 4 295 4H335C354 4 370 8 384 18V94C384 103 377 110 368 110H16C7 110 0 103 0 94Z"}
              fill="var(--background)"
            ...
            <path
              d={frontShape === "flat"
                ? "M22 11H362C369 11 374 16 374 23V89C374 97 369 101 360 101H24C15 101 10 96 10 89V23C10 16 15 11 22 11Z"
                : "M10 21C22 13 35 11 51 11H85C105 11 120 23 137 37L153 50C175 68 208 68 231 49L246 36C262 23 275 11 297 11H333C350 11 363 14 374 22V89C374 97 369 101 360 101H24C15 101 10 96 10 89Z"}
              fill="none"
              stroke="var(--foreground)"
              strokeDasharray="5 5"
              strokeLinecap="round"
          ...
          {front ? (
            <span className="absolute inset-x-[5%] inset-y-0 z-10 block min-w-0">
              {front}
            </span>
          ) : (
