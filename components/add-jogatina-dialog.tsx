...
export function AddJogatinaDialog({ gameId }: AddJogatinaDialogProps) {
  ...
  const addPlayerStatus = () => {
    ...
    setPlayerStatuses([...playerStatuses, { playerId: "", status: "Dava pra jogar", notes: "" }])
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: playersError } = await supabase.from("jogatina_players").insert(
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>Registre uma sessão de jogo com seus amigos</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="border-t pt-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button type="button" variant="outline" size="sm" onClick={addPlayerStatus}>
                  <Plus className="h-4 w-4 mr-2" />
                  Adicionar Jogador
                </Button>
              </div>
              ...
              {playerStatuses.length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-4">
                  Nenhum jogador adicionado. Clique em "Adicionar Jogador" para começar.
                </p>
              )}
                      ...
                      <div className="flex items-start gap-3">
                        <div className="flex-1 space-y-3">
                          ...
                          <div className="grid gap-2">
                            ...
                            <Select
                              value={ps.status}
                              onValueChange={(value) => updatePlayerStatus(index, "status", value)}
                            >
                              <SelectTrigger>
                                <SelectValue />
                              </SelectTrigger>
                        ...
                        <Button type="button" variant="ghost" size="sm" onClick={() => removePlayerStatus(index)}>
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>
