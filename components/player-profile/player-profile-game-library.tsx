...
export function PlayerProfileGameLibrary({
  ...
  if (games.length === 0) {
    return (
      <PlayerProfilePanel className="py-10 text-center text-muted-foreground">
  ...
  return (
    <section>
      {!compact && <PlayerProfileSectionHeader title="Biblioteca" />}
      <PlayerProfilePanel padding={compact ? "compact" : "default"}>
        <div
          className={
            compact
              ? "max-h-[min(380px,55vh)] space-y-2 overflow-y-auto pr-1 [scrollbar-width:thin]"
              : "max-h-[min(560px,65vh)] space-y-2 overflow-y-auto pr-1 [scrollbar-width:thin]"
          }
        >
          {games.map((game) => {
            ...
            return (
              <article
                ...
                className={
                  compact
                    ? "flex items-center gap-2.5 rounded-xl border border-border/40 bg-background/30 p-2"
                    : "flex items-center gap-4 rounded-xl border border-border/40 bg-background/30 p-3"
                }
              >
              <div
