...
export function LandingGroupMetrics({ jogatinaPlayers, seasonParticipants }: LandingGroupMetricsProps) {
  ...
  const statusCounts = {
    ...
    davaJogar: jogatinaPlayers.filter((jp) => jp.status === "Dava pra jogar").length,
  }
  ...
  const avgDuration =
    jogatinaPlayers.length > 0
      ? (
          jogatinaPlayers.reduce((acc, jp) => acc + (jp.total_duration_minutes || 0), 0) / jogatinaPlayers.length
        ).toFixed(0)
      : 0
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground mt-1">Taxa de drop: {dropRate}%</p>
        </CardHeader>
        <CardContent>
      ...
      <div className="space-y-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">minutos por participação</p>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Card className="p-4 border-red-500/30 bg-red-500/5">
            ...
            <p className="text-2xl font-bold text-red-500">{statusCounts.dropo}</p>
          </Card>
        </div>
      </div>
    </div>
